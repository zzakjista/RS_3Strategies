...
class AbstractDataset(metaclass=ABCMeta):
    ...
    def partial_data(self, df):
        ...
        df = df.sample(frac=self.ratio, random_state=self.sampling_seed) # 10%의 행만 랜덤하게 샘플링
        return df
    ...
    def sim_augmentation(self, user2items):
        ...
        for usr_idx in list(user2items.index): 
            seq = user2items[usr_idx][-self.max_len:-2] #max_len만큼의 시퀀스
    ...
    def _get_preprocessed_folder_path(self):
        ...
        folder_name = 'type_{}-size_{}-p{}-drop{}-embedding_{}-data_ratio_{}' \
            .format(self.args.data_type, self.args.N_Aug, self.args.P, self.args.bert_dropout, self.args.model_embedding,self.ratio)
